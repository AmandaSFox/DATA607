...
#-------- Create more dataframes: # SELECT * FROM <tbl> 

db_job_posting_skills <- dbSendQuery(db_con,paste0("SELECT * FROM `project_3_team`.tbl_job_posting_skills;"))
df_job_posting_skills <- fetch(db_job_posting_skills,n=-1)

str(df_job_posting_skills)  
summary(df_job_posting_skills)


db_skills_master <- dbSendQuery(db_con,paste0("SELECT * FROM `project_3_team`.tbl_skills_master;"))
df_skills_master <- fetch(db_skills_master,n=-1)

str(df_skills_master)  
summary(df_skills_master)

# --------- Test join on skill id

df_test2 <- mutate(left_join(df_job_posting_skills, df_skills_master, by = "skill_id"))
